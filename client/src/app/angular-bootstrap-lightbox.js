...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=lightbox-nav><a ng-href={{Lightbox.imageUrl}} target=_blank title=\"Open in new tab\">Open image in new tab</a> </div></div><div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img lightbox-src={{Lightbox.imageUrl}} alt=\"\"><div class=\"lightbox-nav-overlay\" style=\"display: block;\"><a href=\"#\" ng-click=Lightbox.prevImage() class=\"glyphicon glyphicon-chevron-left modal-animate\"></a><a href=\"#\"  ng-click=Lightbox.nextImage() class=\"glyphicon glyphicon-chevron-right modal-animate\"></a></div></div></div>"
  );
}]);
/**
 * @class     ImageLoader
    ...
    'link': function (scope, element, attrs) {
      ...
      var resize = function () {
        ...
        element.css({
          ...
          'line-height': imageDisplayDimensions.height + 'px'
        });
